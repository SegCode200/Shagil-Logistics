...
const reasons = [
  ...
  "Rider requested extra money",
  "Rider damaged my package",
  "Rider did not follow instructions",
  "Rider was too late",
...
export function DeliveryFeedback({
  ...
  order: {
    ...
    riderRating?: { rating: number; comment?: string | null } | null;
    report?: { id: string } | null;
    riderReport?: { id: string } | null;
  };
}) {
  ...
  const riderName = order.rider?.name || order.assignedRider?.name;
  const hasReport = Boolean(order.report || order.riderReport);
  const ratingMutation = useMutation({
    mutationFn: () =>
      api.submitDeliveryRating(token, {
        rating,
  ...
  const reportMutation = useMutation({
    mutationFn: () =>
      api.submitDeliveryReport(token, {
        reason,
    ...
    onSuccess: () => {
      setShowReport(false);
      setMessage(
        "Report submitted successfully. Our team will review your report.",
      );
      queryClient.invalidateQueries({ queryKey: ["customer-delivery", token] });
    },
  });
  if (order.status !== "DELIVERED")
    return (
      <section className="feedback-note">
        Feedback will be available after your delivery is completed.
      </section>
    );
  return (
    <section className="feedback-stack">
      {savedRating ? (
        <div className="feedback-card">
          <h2>Your rating</h2>
          <Stars value={savedRating.rating} />
        ...
        <div className="feedback-card">
          ...
          <label className="feedback-label" htmlFor="rating-comment">
            How was your experience? <span>(optional)</span>
          </label>
          <textarea
            className="textarea"
            id="rating-comment"
      ...
      {riderName && (
        <div className="feedback-rider">
          Your rider<strong>{riderName}</strong>
        </div>
      )}
      {!hasReport && (
        <div className="feedback-card report-card">
          <div>
            <h2>Report a problem</h2>
            <p>If something went wrong, tell our team what happened.</p>
          </div>
          <button
            className="button button-secondary"
            onClick={() => setShowReport(true)}
          >
            <MessageSquareWarning size={17} /> Report rider
          </button>
        </div>
      )}
      {showReport && (
        <div className="feedback-modal-backdrop">
          ...
          >
            ...
            <h2 id="report-title">Report an issue with the rider</h2>
            <div className="field">
            ...
            <div className="field">
              ...
              <textarea
                ...
                placeholder={
                  reason === "Other"
                    ? "Please provide details so we can help"
    ...
    <div className="rating-stars" aria-label={`${value} out of 5 stars`}>
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          key={star}
          type="button"
          className={star <= value ? "star selected" : "star"}
